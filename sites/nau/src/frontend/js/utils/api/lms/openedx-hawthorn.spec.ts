...
describe('OpenEdX Hawthorn API', () => {
  ...
  describe('enrollment', () => {
    ...
    describe('set', () => {
      ...
      it('throws HttpError code 400 with a custom message if request fails', async () => {
        fetchMock.post(`${EDX_ENDPOINT}/api/enrollment/v1/enrollment`, {
          status: 400,
          body: {
            message: 'You are not authorized to enrolled in this course',
          },
        });
        ...
        ).rejects.toThrow(new HttpError(400, 'You are not authorized to enrolled in this course'));
      });
    });
  });
});
