...
locals {
  s3_static_origin_id = "nau-${var.site}-static-origin"
  s3_media_origin_id = "nau-${var.site}-media-origin"
}
...
resource "aws_cloudfront_origin_access_identity" "richie_oai" {
  comment = "nau ${var.site} origin for the ${terraform.workspace} environment"
}
